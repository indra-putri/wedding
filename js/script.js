...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ---------- 7. RSVP & UCAPAN ---------- */
  // RSVP & Ucapan memakai Google Form yang di-embed langsung lewat <iframe>
  // di index.html, jadi tidak perlu JavaScript tambahan di sini.
  // Lihat README.md bagian "RSVP & Ucapan (Google Form)" untuk cara memasang
  // link embed Anda sendiri.

  /* ---------- 8. MEMPELAI: TOMBOL "SELENGKAPNYA" ---------- */
  var moreButtons = document.querySelectorAll('.couple-more-btn');
  moreButtons.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      btn.firstChild.textContent = isHidden ? 'Sembunyikan ' : 'Selengkapnya ';
    });
  });

  /* ---------- 9. TANDA KASIH: TOMBOL "TAMPILKAN REKENING" ---------- */
  var revealBtn = document.getElementById('reveal-gift-btn');
  var giftAccounts = document.getElementById('gift-accounts');
  if (revealBtn && giftAccounts) {
    revealBtn.addEventListener('click', function () {
      var isHidden = giftAccounts.hidden;
      giftAccounts.hidden = !isHidden;
      revealBtn.setAttribute('aria-expanded', String(isHidden));
      revealBtn.textContent = isHidden ? 'Sembunyikan Rekening' : 'Tampilkan Rekening';
      if (isHidden) {
        giftAccounts.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
      }
    });
  }

  /* ---------- 10. TOMBOL "SIMPAN TANGGAL" (unduh file .ics) ---------- */
  var saveDateBtn = document.getElementById('save-date-btn');
  if (saveDateBtn) {
    saveDateBtn.addEventListener('click', function () {
      var start = '20261012T000000';
      var end = '20261013T000000';
      var icsContent = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//Undangan Indra & Putri//ID',
        'CALSCALE:GREGORIAN',
        'BEGIN:VEVENT',
        'UID:pawiwahan-indra-putri-' + Date.now() + '@undangan',
        'DTSTAMP:' + start + 'Z',
        'DTSTART;VALUE=DATE:20261012',
        'DTEND;VALUE=DATE:20261013',
        'SUMMARY:Pawiwahan Indra & Putri',
        'DESCRIPTION:Upacara Pawiwahan & Resepsi Indra & Putri. Pukul 08.00 - 18.00 WITA.',
        'LOCATION:Jl. Veteran\\, Gg. Pura Beji\\, Desa Pelapuan\\, Kecamatan Busungbiu\\, Buleleng\\, Bali',
        'END:VEVENT',
        'END:VCALENDAR'
      ].join('\r\n');
      ...
      a.href = url;
      a.download = 'Pawiwahan-Indra-Putri.ics';
